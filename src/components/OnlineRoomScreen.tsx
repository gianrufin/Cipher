...
export const OnlineRoomScreen = ({ onBack }: { onBack: () => void }) => {
  ...
  const startMatch = async () => {
    ...
    const category = settings.categoryId === 'variety'
      ? { id: 'variety', name: 'Variety Deck', iconName: 'Sparkles', description: 'All easy topics.', pairs: BUILT_IN_CATEGORIES.flatMap(item => item.pairs.filter(pair => pair.difficulty === 'easy')) }
      : BUILT_IN_CATEGORIES.find(item => item.id === settings.categoryId) || BUILT_IN_CATEGORIES[0];
