...
const ROLE_COPY: Record<OnlineAssignment['role'], string> = {
  ...
  anarchist: 'Bait the table into ejecting you first.'
};

const defaultSettings: OnlineSettings = { imposters: 1, decoys: 0, ejectionReveal: 'confirm', allowSkip: true, eliminations: 1, categoryId: 'pinoy_everyday' };
...
export const OnlineRoomScreen = ({ onBack }: { onBack: () => void }) => {
  ...
  const finalizeEjection = (targetId: string) => {
    ...
    if (targetRole === 'anarchist' && ejectionPositionRef.current === 0) {
      publishState({ ...current, players: nextPlayers, ejectedId: targetId, ejectionText: `${target.name} was the Anarchist.`, phase: 'ejection' });
  ...
  const startMatch = () => {
    ...
    const category = BUILT_IN_CATEGORIES.find(item => item.id === settings.categoryId) || BUILT_IN_CATEGORIES[0];
    const easyPairs = category.pairs.filter(pair => pair.difficulty === 'easy');
    const pair = easyPairs[Math.floor(Math.random() * easyPairs.length)] || category.pairs[0];
    ...
    assignmentsRef.current = generated.assignments; citizenWordRef.current = generated.citizenWord; bodyguardUsedRef.current = false; activePromptRef.current = undefined; setRoleVisible(false); setCounterWord('');
  ...
  if (status === 'idle' || status === 'error') return <JoinRoom role={role} setRole={setRole} name={name} setName={setName} code={code} setCode={setCode} photo={photo} notice={notice} onBack={onBack} onSelfie={() => setSelfieOpen(true)} onConnect={connect} selfieOpen={selfieOpen} setSelfieOpen={setSelfieOpen} setPhoto={setPhoto} />;
  ...
  return <div className="mx-auto w-full max-w-lg px-4 py-8">
    ...
    {match?.phase === 'reveal' && assignment && <section className="cipher-panel mt-6 p-6 text-center"><p className="cipher-kicker">Private evidence / eyes only</p><PlayerAvatar name={name} src={photo} className="mx-auto mt-5 h-24 w-24"/><h2 className="mt-4 text-2xl font-black text-stone-100">{roleVisible ? assignment.role : 'Role encrypted'}</h2>{roleVisible ? <div className="mt-4 space-y-3"><p className="font-display text-4xl font-black text-rose-300">{assignment.word}</p><p className="text-sm text-stone-400">{ROLE_COPY[assignment.role]}</p>{assignment.intel && <p className="rounded-xl bg-sky-500/10 p-3 text-xs text-sky-300">{assignment.intel}</p>}{assignment.teammates.length > 0 && <p className="text-xs text-stone-400">Teammates: {assignment.teammates.join(', ')}</p>}</div> : <p className="mt-3 text-xs text-stone-500">Shield your screen before opening the file.</p>}<button onClick={() => setRoleVisible(value => !value)} className="cipher-button-secondary mt-5 w-full"><Eye className="h-4 w-4" /> {roleVisible ? 'Hide role' : 'Reveal role'}</button><button disabled={!roleVisible || match.readyIds.includes(peerId.current)} onClick={() => sendAction({ type: 'ready', playerId: peerId.current })} className="cipher-button-primary mt-3 w-full disabled:opacity-30"><Check className="h-4 w-4" /> {match.readyIds.includes(peerId.current) ? 'Ready · waiting for crew' : 'I understand my role'}</button></section>}

    {match?.phase === 'clues' && <section className="cipher-panel mt-6 p-6 text-center"><p className="cipher-kicker">Round {match.round} / clue sequence</p><PlayerAvatar name={currentSpeaker?.name || ''} src={currentSpeaker?.photo} className="mx-auto mt-5 h-24 w-24"/><h2 className="mt-4 text-3xl font-black text-stone-100">{currentSpeaker?.id === peerId.current ? 'Your clue.' : `${currentSpeaker?.name} is speaking.`}</h2><p className="mt-3 text-sm text-stone-500">Category: {match.categoryName}. Give one helpful clue without saying either word.</p>{currentSpeaker?.id === peerId.current && <button onClick={() => sendAction({ type: 'clue-done', playerId: peerId.current })} className="cipher-button-primary mt-5 w-full"><Send className="h-4 w-4" /> Clue given</button>}</section>}

    {match?.phase === 'voting' && me && !me.eliminated && <section className="cipher-panel mt-6 p-5"><p className="cipher-kicker">{match.runoffCandidateIds ? 'Silent runoff' : `Round ${match.round} / silent ballot`}</p><h2 className="mt-2 text-2xl font-black text-stone-100">Pick a suspect.</h2><div className="mt-4 grid grid-cols-3 gap-2">{candidates.map(player => <button key={player.id} disabled={match.votedIds.includes(peerId.current)} onClick={() => { setSelectedVote(player.id); sendAction({ type: 'vote', voterId: peerId.current, targetId: player.id }); }} className={`rounded-2xl border p-2 disabled:opacity-60 ${selectedVote === player.id ? 'border-rose-500 bg-rose-500/10' : 'border-white/10'}`}><PlayerAvatar name={player.name} src={player.photo} className="aspect-square w-full"/><span className="mt-2 block truncate text-[10px] text-stone-100">{player.name}</span></button>)}</div>{match.allowSkip && !match.runoffCandidateIds && <button disabled={match.votedIds.includes(peerId.current)} onClick={() => { setSelectedVote('skip'); sendAction({ type: 'vote', voterId: peerId.current }); }} className="cipher-button-ghost mt-3 w-full"><FastForward className="h-4 w-4" /> Skip</button>}<p className="mt-4 text-center text-xs text-stone-500">{match.votedIds.includes(peerId.current) ? 'Ballot locked. Results stay hidden until everyone votes.' : `${match.votedIds.length} of ${activePlayers(match).length} ballots locked`}</p></section>}
