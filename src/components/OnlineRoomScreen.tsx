...
export const OnlineRoomScreen = ({ onBack, initialCrew }: { onBack: () => void; initialCrew?: CrewProfile }) => {
  ...
  useEffect(() => {
    ...
    return () => { document.removeEventListener('visibilitychange', hidePrivateState); window.removeEventListener('blur', blur); };
  }, []);
  ...
  return <div className="mx-auto w-full max-w-lg px-4 py-8">
    ...
    {!match && <Lobby role={role} roster={roster} settings={settings} setSettings={setSettings} onStart={startMatch} onRemove={removePlayer} myId={peerId.current} notice={notice} />}

    {match?.phase === 'reveal' && assignment && <section className="cipher-panel mt-6 p-6 text-center"><p className="cipher-kicker">Private role · {match.readyIds.length} of {activePlayers(match).length} ready</p><PlayerAvatar name={name} src={photo} className="mx-auto mt-5 h-24 w-24"/><h2 className="mt-4 text-2xl font-black text-stone-100">{roleVisible ? assignment.role : `Private card for ${name}`}</h2>{roleVisible ? <div className="mt-4 space-y-3"><p className="font-display text-5xl font-black text-rose-300">{assignment.word}</p><p className="text-sm text-stone-400">{ROLE_COPY[assignment.role]}</p>{assignment.intel && <p className="rounded-xl bg-sky-500/10 p-3 text-xs text-sky-300">{assignment.intel}</p>}{assignment.teammates.length > 0 && <p className="text-xs text-stone-400">Teammates: {assignment.teammates.join(', ')}</p>}</div> : <p className="mt-3 text-xs text-stone-500">{roleViewed ? 'Hold again if you need another look.' : 'Shield your screen, then press and hold.'}</p>}<button onPointerDown={() => { setRoleVisible(true); setRoleViewed(true); }} onPointerUp={() => setRoleVisible(false)} onPointerCancel={() => setRoleVisible(false)} onPointerLeave={() => setRoleVisible(false)} className="cipher-button-secondary mt-5 w-full touch-none"><Eye className="h-4 w-4" /> {roleVisible ? 'Release to hide' : 'Press and hold to view'}</button><button disabled={!roleViewed || roleVisible || match.readyIds.includes(peerId.current)} onClick={() => sendAction({ type: 'ready', playerId: peerId.current })} className="cipher-button-primary mt-3 w-full disabled:opacity-30"><Check className="h-4 w-4" /> {match.readyIds.includes(peerId.current) ? 'Ready · waiting for others' : 'Hide card and mark ready'}</button></section>}
    ...
    {match?.phase === 'results' && <section className="cipher-panel mt-6 p-6 text-center"><p className="cipher-kicker">Case closed</p><h2 className="mt-2 font-display text-4xl font-black capitalize text-stone-100">{match.winner} win</h2><p className="mt-3 text-sm text-stone-500">{match.winReason}</p><div className="mt-5 space-y-2">{[...match.players].sort((a,b) => b.score-a.score).map((player,index) => <div key={player.id} className="flex items-center gap-3 rounded-xl border border-white/10 p-3 text-left"><span className="font-mono text-xs text-stone-500">#{index+1}</span><PlayerAvatar name={player.name} src={player.photo} className="h-11 w-11"/><strong className="flex-1 text-stone-100">{player.name}</strong><span className="font-mono text-amber-300">{player.score} pts</span></div>)}</div>{role === 'host' && <div className="mt-5 grid grid-cols-[1fr_auto] gap-2"><button onClick={startMatch} className="cipher-button-primary">Same group · fresh words</button><button onClick={() => setInviteOpen(true)} className="cipher-icon-button" aria-label="Invite players"><QrCode className="h-5 w-5"/></button></div>}</section>}
...
const Lobby = ({ role, roster, settings, setSettings, onStart, onRemove, myId, notice }: { role: 'host'|'player'; roster: OnlineProfile[]; settings: OnlineSettings; setSettings: React.Dispatch<React.SetStateAction<OnlineSettings>>; onStart: () => void; onRemove: (id:string)=>void; myId:string; notice: string }) => { const online=roster.filter(player=>player.connected).length; return <div className="mt-6"><div className="grid grid-cols-3 gap-2 sm:grid-cols-4">{roster.map(profile => <div key={profile.id} className="evidence-note relative rounded-2xl border border-white/10 p-2 text-center"><PlayerAvatar name={profile.name} src={profile.photo} className="aspect-square w-full"/><span className="mt-2 block truncate text-[11px] font-bold text-stone-100">{profile.name}</span><span className={`text-[8px] uppercase ${profile.connected ? 'text-emerald-300' : 'text-stone-600'}`}>{profile.connected ? 'ready' : 'reconnecting'}</span>{role==='host'&&profile.id!==myId&&<button onClick={()=>onRemove(profile.id)} className="absolute right-1 top-1 grid h-7 w-7 place-items-center rounded-full bg-stone-950/75 text-stone-300" aria-label={`Remove ${profile.name}`}><UserMinus className="h-3.5 w-3.5"/></button>}</div>)}</div>{role === 'host' ? <section className="cipher-panel mt-5 space-y-4 p-5"><div><p className="cipher-kicker">Host settings</p><p className="mt-2 text-xs text-stone-500">{online} joined · {settings.imposters} Imposter{settings.imposters===1?'':'s'} · {settings.eliminations} ejection{settings.eliminations===1?'':'s'} per vote</p></div><div className="grid grid-cols-2 gap-2"><SettingSelect label="Imposters" value={settings.imposters} values={[1,2,3]} onChange={value => setSettings(current => ({...current,imposters:Number(value) as 1|2|3}))}/><SettingSelect label="Decoys" value={settings.decoys} values={[0,1,2]} onChange={value => setSettings(current => ({...current,decoys:Number(value) as 0|1|2}))}/><SettingSelect label="Ejections" value={settings.eliminations} values={[1,2]} onChange={value => setSettings(current => ({...current,eliminations:Number(value) as 1|2}))}/><SettingSelect label="Reveal" value={settings.ejectionReveal} values={['confirm','classified']} onChange={value => setSettings(current => ({...current,ejectionReveal:value as 'confirm'|'classified'}))}/></div><label className="flex items-center justify-between text-xs text-stone-300">Allow skip<input type="checkbox" checked={settings.allowSkip} onChange={event => setSettings(current => ({...current,allowSkip:event.target.checked}))}/></label><label className="block text-xs text-stone-400">Easy word pack<select className="cipher-input mt-2 w-full" value={settings.categoryId} onChange={event => setSettings(current => ({...current,categoryId:event.target.value}))}><option value="variety">Variety Deck · 40 pairs</option>{BUILT_IN_CATEGORIES.filter(category => category.pairs.some(pair => pair.difficulty === 'easy')).map(category => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label>{notice && <p className="text-xs text-rose-300">{notice}</p>}<button onClick={onStart} disabled={online<4} className="cipher-button-primary w-full disabled:opacity-30"><Send className="h-4 w-4" /> {online<4?`Need ${4-online} more player${4-online===1?'':'s'}`:`Start match with ${online}`}</button></section> : <div className="cipher-panel mt-5 p-6 text-center"><Radio className="mx-auto h-7 w-7 text-sky-300"/><p className="mt-3 text-sm text-stone-400">You are ready. Waiting for the host to start.</p></div>}</div>; };
