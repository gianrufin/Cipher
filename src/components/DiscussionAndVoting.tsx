...
export const DiscussionAndVoting: React.FC<DiscussionAndVotingProps> = ({ players, initialVotingStyle = 'open', eliminationsPerVote, allowSkip, onEliminatePlayers, onReturnToClues, onSkipVote }) => {
  ...
  const selectOpenTarget = (playerId: string) => {
    const next = selectedTargetIds.includes(playerId)
      ? selectedTargetIds.filter(id => id !== playerId)
      : selectedTargetIds.length < selectionCount ? [...selectedTargetIds, playerId] : selectedTargetIds;
    if (next.length > selectedTargetIds.length) playVote();
    setSelectedTargetIds(next);
    if (next.length === selectionCount) setShowOpenConfirmation(true);
  };
  const selectSecretTarget = (playerId: string) => {
    const next = currentSecretSelection.includes(playerId)
      ? currentSecretSelection.filter(id => id !== playerId)
      : currentSecretSelection.length < requiredSelections ? [...currentSecretSelection, playerId] : currentSecretSelection;
    if (next.length > currentSecretSelection.length) playVote();
    setCurrentSecretSelection(next);
    if (next.length === requiredSelections) { setPendingSkip(false); setSecretStep('confirm'); }
  };
  ...
  return <div className="mx-auto w-full max-w-lg space-y-5 px-4 pb-24 pt-2">
    ...
    {votingMethod === 'open' && <div className="space-y-4">
      ...
      <PlayerChoices players={activePlayers} selectedIds={selectedTargetIds} onSelect={selectOpenTarget} />
      {selectedTargetIds.length > 0 && <div className="rounded-2xl border border-rose-500/40 bg-[#160c10] p-4 text-center"><p className="font-mono text-[10px] font-bold uppercase tracking-widest text-rose-300">Selected</p><h3 className="mt-2 font-display text-lg font-bold text-slate-100">{selectedTargetIds.map((id, index) => `${index + 1}. ${activePlayers.find(player => player.id === id)?.name}`).join('  ·  ')}</h3></div>}
    ...
    {votingMethod === 'secret' && <div className="space-y-4">
      ...
      {secretStep === 'vote' && currentVoter && <div className="space-y-3"><div className="flex justify-between text-xs text-slate-400"><span>{currentVoter.name}'s ballot</span><span className="font-mono text-rose-400">SELECT {requiredSelections}</span></div><PlayerChoices players={availableCandidates} selectedIds={currentSecretSelection} disabledId={currentVoter.id} onSelect={selectSecretTarget} />{allowSkip && !isRunoff && <button type="button" onClick={() => { setPendingSkip(true); setSecretStep('confirm'); }} className="cipher-button-ghost w-full"><FastForward className="h-4 w-4" /> Skip ballot</button>}</div>}
      {secretStep === 'confirm' && currentVoter && <section className="vote-confirmation"><p className="cipher-eyebrow">Check your ballot</p><h3 className="mt-3 font-display text-3xl font-black">{pendingSkip ? 'Submit a blank ballot?' : 'Confirm your vote?'}</h3><p className="mt-3 text-sm text-[var(--muted)]">{pendingSkip ? 'You are choosing not to accuse anyone.' : currentSecretSelection.map((id, index) => `${requiredSelections > 1 ? `${index + 1}. ` : ''}${activePlayers.find(player => player.id === id)?.name}`).join('  ·  ')}</p><div className="mt-6 grid grid-cols-2 gap-2"><button className="cipher-button-secondary" onClick={() => { setCurrentSecretSelection([]); setPendingSkip(false); setSecretStep('vote'); }}>Change</button><button className="cipher-button-primary" onClick={() => handleSecretVoteSubmit(pendingSkip)}>{pendingSkip ? 'Confirm skip' : 'Confirm vote'}</button></div></section>}
    ...
    {showOpenConfirmation && createPortal(<div className="vote-popup" role="dialog" aria-modal="true" aria-label="Confirm vote"><section><p className="cipher-eyebrow">Are you sure?</p><h3 className="mt-2 font-display text-3xl font-black">Confirm your vote</h3><p className="mt-3 text-sm text-[var(--muted)]">{selectedTargetIds.map((id,index)=>`${selectionCount>1?`${index+1}. `:''}${activePlayers.find(player=>player.id===id)?.name}`).join(' · ')}</p><div className="mt-6 grid grid-cols-2 gap-2"><button onClick={()=>{setSelectedTargetIds([]);setShowOpenConfirmation(false);}} className="cipher-button-secondary">Change</button><button onClick={confirmOpenVote} className="cipher-button-primary">Confirm vote</button></div></section></div>, document.body)}
  </div>;
};
