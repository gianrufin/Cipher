...
export const DiscussionAndVoting: React.FC<DiscussionAndVotingProps> = ({ players, initialVotingStyle = 'open', eliminationsPerVote, allowSkip, onEliminatePlayers, onReturnToClues, onSkipVote }) => {
  ...
  return <div className="mx-auto w-full max-w-lg space-y-5 px-4 pb-24 pt-2">
    ...
    {showOpenConfirmation && createPortal(<div className="vote-popup" role="dialog" aria-modal="true" aria-label="Confirm vote"><section><p className="cipher-eyebrow">Are you sure?</p><h3 className="mt-2 font-display text-3xl font-black">Confirm your vote</h3><p className="mt-3 text-sm text-[var(--muted)]">{selectedTargetIds.map((id,index)=>`${selectionCount>1?`${index+1}. `:''}${activePlayers.find(player=>player.id===id)?.name}`).join(' · ')}</p><div className="mt-6 grid grid-cols-2 gap-2"><button onClick={()=>setShowOpenConfirmation(false)} className="cipher-button-secondary">Change</button><button onClick={confirmOpenVote} className="cipher-button-primary">Confirm vote</button></div></section></div>, document.body)}
  </div>;
};
